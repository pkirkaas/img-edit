...
- Config: [src/lib/config.py](src/lib/config.py) — environment-backed settings (tokens, provider, model, endpoint, timeouts)
- Client: [src/lib/hf_client.py](src/lib/hf_client.py) — thin wrapper over huggingface_hub.InferenceClient (no custom HTTP)
- Pipeline: [src/lib/edit_pipeline.py](src/lib/edit_pipeline.py) — orchestrates client calls and result handling
